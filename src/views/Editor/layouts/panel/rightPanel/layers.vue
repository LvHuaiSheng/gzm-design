  ...
  for (const object of objects) {
    ...
    const children = isCollection
      ? getTreeData(
          object.children,
          searchKey,
          parentVisible && proxyData.visible
        )
      : [];
...
const moveNode = (data: {
  ...
  // 画板不能进组
  if (
    (dragGroup !== dropGroup || dropPosition === 0) &&
    canvas.objectIsTypes(dragObject, "Leafer")
  ) {
    return false;
  }
...
const onDrop = (data: DropEvent) => {
  const { e, dragNode, dropNode, dropPosition } =
    data as DropEvent<ITreeNodeData>;
  ...
  try {
    ...
    // 多个拖拽
    if (
      selectedkeys.value.includes(dragNode.key) &&
      selectedkeys.value.length > 1
    ) {
      // 获取所有要拖拽的对象（不包括dropNode）
      const dragKeys = selectedkeys.value.filter((key) => key !== dropNode.key);
      const dragObjects = canvas.findObjectsByIds(dragKeys);
      const dropObject = canvas.findObjectById(dropNode.key);

      if (!isDefined(dropObject) || dragObjects.length === 0) return;

      // 根据dropPosition决定处理顺序以保持相对顺序
      // dropPosition === -1: 从后往前处理（放在目标之前）
      // dropPosition === 0 或 1: 从前往后处理（放在目标之后或内部）
      if (dropPosition === -1) {
        // 从后往前处理，保持相对顺序
        for (let i = dragObjects.length - 1; i >= 0; i--) {
          const dragObject = dragObjects[i];
          if (!isDefined(dragObject)) continue;
          moveNode({
            e,
            dragObject,
            dropObject,
    ...
    // 单个拖拽
      ...
      const dragObject = canvas.findObjectById(dragNode.key);
      const dropObject = canvas.findObjectById(dropNode.key);
...
const updateSelectedkeys = async () => {
  ...
  if (treeRef.value && treeRef.value.virtualListRef) {
    const containerRect =
      treeRef.value?.virtualListRef.containerRef.getBoundingClientRect();
...
<template>
  <div ref="splitRef" class="h-[calc(100vh-90px)]">
