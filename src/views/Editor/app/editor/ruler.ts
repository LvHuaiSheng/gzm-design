...
export class Ruler extends Disposable {
  ...
  constructor(
    ...
    watchEffect(() => {
      ...
      this.render({ ctx: this.canvas.app.canvas.context })
    })
  ...
  public set enabled(value) {
    ...
    if (value) {
      ...
      this.canvas.app.off(RenderEvent.AFTER,arg => {
        this.render({ctx: this.canvas.app.canvas.context})
      })
  ...
  private render({ ctx }: { ctx: ICanvasContext2D }) {
    if (ctx !== this.canvas.app.canvas.context) return
    this.canvas.app.worldTransform
    const { worldTransform } = this.canvas.app
  ...
  private calcObjectRect() {
    ...
    const allRect = activeObjects.reduce((rects, obj) => {
      const bounds = obj.worldBoxBounds
